...
int main()
{
    ...
    check_error = ftruncate(fd, SIZE);
    if (check_error == -1) { // задаем ей нужный размер (для разделяемой памяти так можно, для обычных файлов -- не всегда. Для низ лучше lseek, write)
